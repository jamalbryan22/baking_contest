...
@app.route("/send-delete-message", methods=["GET", "POST"])
def send_delete_message():
    if request.method == "POST":
        ...
        # Validate EntryId
        if not entry_id.isdigit() or int(entry_id) <= 0:
            return render_template("results.html", msg="Error: EntryId must be a numeric value greater than 0.")

        # Encrypt and send the message
        message = f"EntryId={entry_id}"
        encrypted_message = encrypt_message(message, SECRET_KEY)
        hmac_signature = hmac.new(SECRET_KEY, encrypted_message, hashlib.sha3_512).digest()

        try:
            with socket.create_connection((HOST, PORT)) as client_socket:
                client_socket.sendall(hmac_signature + encrypted_message)
                response = client_socket.recv(1024).decode()  # Receive server response
                return render_template("results.html", msg=response)
        except Exception as e:
            return render_template("results.html", msg=f"Error: {str(e)}")

    return render_template("send_delete_message.html")
